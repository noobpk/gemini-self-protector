...
class _Config(object):
    ...
    def get_tb_behavior_log() -> None:
        try:
            session = _Config.get_session()
            behaviors = _Config.get_model_instance_all(session, tb_BehaviorLog)
            return behaviors
        except Exception as e:
            logger.error(
                "[x_x] Something went wrong at {0}, please check your error message.\n Message - {1}".format(
                    "_Config.get_tb_behavior_log", e
                )
            )
    ...
    def store_tb_behavior_log(
        ...
        try:
            ...
            logger.error(
                "[x_x] Something went wrong at {0}, please check your error message.\n Message - {1}".format(
                    "_Config.store_tb_behavior_log", e
                )
            )
    ...
    def get_tb_request_log_first(event_id) -> None:
        try:
            session = _Config.get_session()
            req_record = (
                session.query(tb_RequestLog).filter_by(event_id=event_id).first()
            )
            session.close()
            return req_record
        except Exception as e:
    ...
    def update_record_request_log(_event_id) -> None:
        try:
            session = _Config.get_session()
            request_log_record = (
                session.query(tb_RequestLog).filter_by(event_id=_event_id).first()
            )
    ...
    def store_tb_acl(ipaddress, isallow, desciption):
        try:
            ...
            new_acl = tb_AccessControlList(
                ipaddress=ipaddress, is_allowed=isallow, desciption=desciption
            )
            session.add(new_acl)
            session.commit()
            session.close()
    ...
    def check_acl(_ip_address) -> None:
        try:
            session = _Config.get_session()
            acl_record = (
                session.query(tb_AccessControlList)
                .filter_by(ipaddress=_ip_address)
                .first()
            )
            session.close()
    ...
    def store_tb_dependency(package, version, cve_id, severity):
        try:
            ...
            new_record = tb_Dependency(
                package=package, version=version, cve_id=cve_id, severity=severity
            )
            session.add(new_record)
            session.commit()
            session.close()
    ...
    def store_tb_feedback(_sentence, _label):
        try:
            ...
            logger.error(
                "[x_x] Something went wrong at {0}, please check your error message.\n Message - {1}".format(
                    "_Config.store_gemini_feedback", e
                )
            )
